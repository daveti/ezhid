...
volatile static __data char sunkbd_type;
/* flag for pending reset */
volatile static __data char sunkbd_reset;
/* layout flag */
volatile static __data char sunkbd_layout;

/* key buffer */
volatile __data Byte key_buffer[8];

/*
 * transmit queue
 * at the moment only one byte deep and restricted to 0-127
 */
volatile __data char kbd_txmit_me;   /* apply_reset_values */
#endif
...
volatile __data char mbuf0, mbuf1, mbuf2;

/* mouse buffer */
volatile __data char mouse_buffer[8];

/* current state of mouse packet reception */
volatile __data Byte mouse_state;
#endif
...
static __bit kbd_new_keys;             /* apply_reset_values */
/* the next output packet is related to a control transfer after SETUP */
static __bit is_ctrl_out0_kbd;         /* apply_reset_values */
/*
 * remember IDLE mode
 */
static __bit kbd_idle_mode;            /* apply_reset_values */
/*
 * idle duration expressed as multiples of 4 ms
 * default is 125 * 4 ms = 500 ms
 */
static __data Byte kbd_idle_duration;  /* apply_reset_values */
static __data Byte kbd_idle_counter;
/* busy semaphore for IN1 endpoint */
static __bit in1_busy;                 /* apply_reset_values */
#endif

#if defined (MOUSE)
/* report has changed, in2buf should be transferred on the next opportunity */
static __bit mouse_new_keys;           /* apply_reset_values */
/* the next output packet is related to a control transfer after SETUP */
static __bit is_ctrl_out0_mouse;       /* apply_reset_values */
/*
 * remember IDLE mode
 */
static __bit mouse_idle_mode;          /* apply_reset_values */
/*
 * idle duration expressed as multiples of 4 ms
 * default is 5 * 4 ms = 20 ms
 */
static __data Byte mouse_idle_duration; /* apply_reset_values */
static __data Byte mouse_idle_counter;
/* busy semaphore for IN2 endpoint */
static __bit in2_busy;                 /* apply_reset_values */
#endif


/* the device descriptor */
static __code Byte dev_desc[] = {
...
#if defined (KEYBOARD)
static __code Byte report_desc_keyboard[] = {
...
#if defined (MOUSE)
static __code Byte report_desc_mouse[] = {
   0x05, 0x01, // Usage Page (Generic Desktop),
...
static __code Byte string_langid[] = { 0x04, 0x03, 0x00, 0x00 };

/* manufacturer string descriptor */
static __code Byte string_mfg[] = {
   0x22, 0x03,
...
static __code Byte string_prod[] = {
#if defined (KEYBOARD) && !defined (MOUSE)
   0x22, 0x03,
...
static __code Byte string_if_keyboard[] = {
   0x86, 0x03,
   'S',0, 'u',0, 'n',0, ' ',0, 'K',0, 'e',0, 'y',0, 'b',0,
   'o',0, 'a',0, 'r',0, 'd',0, ' ',0, 'H',0, 'I',0, 'D',0,
...
static __code Byte string_if_mouse[] = {
   0x80, 0x03,
   'S',0, 'u',0, 'n',0, ' ',0, 'M',0, 'o',0, 'u',0, 's',0,
...
static __data unsigned int string_index[NUM_STRING] = {
 ...
 */
static __code Byte scancode_map[] = {
...
static __xdata setup_dat * __data sdat = (__xdata setup_dat *)&SETUPDAT;
